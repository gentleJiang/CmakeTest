...
void SVC_HEX_2_BIT(byte *inText, int *outText, int count)
{
	register int i, j;
...
void SVC_BIT_2_HEX(int *inText, byte *outText, int count)
{
	register int i, j;
...
void asc_des(char *dkey, char *tt, char *cipher, char f)
{
	...
	SVC_DSP_2_HEX(dkey, buf, 8);
	SVC_DSP_2_HEX(tt, buf1, 8);
	des((byte *)buf, (byte *)buf1, (byte *)buf2, f);
	SVC_HEX_2_DSP(buf2, cipher, 8);
	cipher[16] = 0;
}*/
...
void TripleDes(byte *dkey, byte *tt, byte *cipher, char f)
{
	...
	if(f == 'e' || f == 'E')
	{
		des(dkey, tt, cipher, 'E');
		des(dkey+8, cipher, buffer, 'D');
		des(dkey, buffer, cipher, 'E');
	}
	else
	{
		des(dkey, tt, cipher, 'D');
		des(dkey+8, cipher, buffer, 'E');
		des(dkey, buffer, cipher, 'D');
	}
}
...
void TripleMac(byte *key, byte *command, int slen, byte *cipher)
{
	...
	for(i = 0; i<len1; i++)
	{
		...
		des(key, buffer1, buffer1, 'e');
	}
	des(key+8, buffer1, buffer2, 'd');
	des(key, buffer2, cipher, 'e');
}
